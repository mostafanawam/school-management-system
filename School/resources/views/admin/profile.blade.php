...
@error('email')<small class="alert text-danger font-weight-bold">{{$message}}</small>@enderror
  ...
  @error('phone')<small class="alert text-danger font-weight-bold">{{$message}}</small>@enderror
    ...
    <div class="form-group col-lg-2">
        <label for="Rank" class="font-weight-bold">Rank:</label>
    <input type="text" name="Rank" value="{{$info->rank}}"  placeholder="Rank" class="form-control">
    @error('Rank')<small class="alert text-danger font-weight-bold">{{$message}}</small>@enderror
      </div>
